...
class Configuration {
	...
	static constexpr int CONTROL_SETPOINT_OFFSET = INITIALIZED_OFFSET + 1;
	static constexpr int CRC_OFFSET = CONTROL_SETPOINT_OFFSET + 2;
...
public:
	...
	uint16_t getControlSetpoint() {
		uint16_t value;
		EEPROM.get(CONTROL_SETPOINT_OFFSET, value);
		return value;
	}
	void setControlSetpoint(uint16_t value) {
		EEPROM.put(CONTROL_SETPOINT_OFFSET, value);
		writeCrc();
	}
