...
class Configuration {
	...
	static constexpr int ALERT_LIGHT_SECONDS_ON = INITIALIZED_OFFSET + 1;
	static constexpr int CRC_OFFSET = ALERT_LIGHT_SECONDS_ON + 2;
...
public:
	...
	uint16_t getAlertLightSecondsOn() {
		uint16_t value;
		EEPROM.get(ALERT_LIGHT_SECONDS_ON, value);
		return value;
	}
	void setAlertLightSecondsOn(uint16_t value) {
		EEPROM.put(ALERT_LIGHT_SECONDS_ON, value);
		writeCrc();
	}
